    ...
    <div class="uk-card-header app--card-header-tool">
      <div class="uk-grid-collapse" uk-grid>
        ...
        <div class="uk-width-expand">
          <h3 class="app--card-title">AGENTS</h3>
        </div>
        <div class="uk-width-auto">
          <div class="app--card-header-tool-button app--border-left">
            <router-link :to="{ name: 'agent-create' }">
    ...
    <div class="uk-card-body">
      <div class="uk-overflow-auto">
        <table class="uk-table uk-table-divider uk-table-small uk-text-small">
          ...
          <tbody>
            <tr v-for="user in agents" :key="user.id">
              <td>{{ user.code }}</td>
              <td>{{ user.name }}</td>
              <td>{{ user.phone }}</td>
              <td>{{ user.email }}</td>
              ...
              <td class="uk-text-center">
                <router-link
                  :to="{ name: 'agent-edit', params: { id: user.id } }"
                >
...
export default {
  ...
  methods: {
    async fetchAgents () {
      ...
      try {
        let res = await this.$service.getAgents(this.filter)
